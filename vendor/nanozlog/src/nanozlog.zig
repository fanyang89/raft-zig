...
pub fn initThreadBuffer(self: *NanoZlog) !void {
...
pub fn start(self: *NanoZlog) !void {
    try self.initDummyBgLogInfos();
    self._is_polling.store(true, .release);
    self._polling_worker = try self._io.concurrent(NanoZlog.pollingWorker, .{self});
}

pub fn rdtsc(self: NanoZlog) i64 {
    return self._tscns.rdtsc();
}

pub fn tsc2ns(self: *NanoZlog, tsc: i64) i64 {
    return self._tscns.tsc2ns(tsc);
}

pub fn rdns(self: *NanoZlog) i64 {
...
fn initDummyBgLogInfos(self: *NanoZlog) !void {
...
fn preallocate(self: *NanoZlog) !void {
...
fn pollingWorker(self: *NanoZlog) void {
                ...
                switch (info) {
                    ...
                    .pointer => |p| {
                        if (p.size == .slice and p.child == u8) {
                            ...
                            @field(args, field.name).ptr = @ptrCast(@constCast(data[idx..].ptr));
                            idx += len;
                            if (p.sentinel() != null) idx += 1;
                        } else if (p.size == .one) {
                            const size = @sizeOf(T);
                            const ptr: *align(1) const T = @ptrCast(data[idx..].ptr);
                            @field(args, field.name) = ptr.*;
...
fn poll(self: *NanoZlog, force_flush: bool) !void {
...
fn adjustHeap(self: NanoZlog, i: usize) void {
    var idx = i;
    while (true) {
        var min_i: usize = idx;
...
fn allocMsg(self: *NanoZlog, size: u32, q_full_cb: bool) !?*SpscVarQueue.MsgHeader {
    try self.preallocate();
